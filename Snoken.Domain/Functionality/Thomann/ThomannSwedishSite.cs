...
{
    internal class ThomannSwedishSite : ISwedishSite

    {
        public string ScrapeSwedishInfo(string url)
        {
            ...
            if (html != null)
            {
                price = html.InnerHtml.Replace(",", ".").Replace("SEK", "").Replace(" ", "") ?? "";
            }
            return price;
        }
        ...
        {
            string url = "http://www.thomann.de/se/search_dir.html?bf=&sw=";
            ...
            if (articleList != null)
                foreach (HtmlNode article in articleList)
                {
                    ...
                    if (id == key)
                    {
                        url = "http://www.thomann.de/se/" + article.SelectSingleNode("//a[@class='lr-articlelist-article-articleLink']").
